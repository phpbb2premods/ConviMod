...
class acp_versions
{
	...
	function read()
	{
		$this->data = array(
			...
			'EzCom' => array(
				...
				'infos' => 'Post_donwload_convimod',
				'host' => 'demo.ezcom-fr.com',
				'file' => '/premod_version_check/convimod_versions.dta',
				'protocol' => 'ezcom',
				'mods' => array(
					'convimod_version' => array('name' => 'Lang_convimod', 'desc' => 'Lang_convimod_explain', 'page' => 'http://www.ezcom-fr.com/viewtopic.php?t=6570'),
				),
			),
		);
	}
	...
	function refresh($force=false)
	{
		...
		foreach ( $this->data as $provider => $info )
		{
			...
			switch ( $info['protocol'] )
			{
				...
				case 'ezcom':
					$this->process_ezcom($provider, $content);
					break;
			}
		}
	...
	function process_ezcom($provider, $content)
	{
		global $board_config;
		...
		for ( $i = 0; $i < $count_content; $i++ )
		{
			...
			$mod_data = array(
				'latest' => trim($line[1]),
				'dev' => trim($line[2]),
				'announcement' => empty($line[3]) ? '' : trim(htmlspecialchars($line[3])),
			);
			if ( !empty($mod_name) && !empty($mod_data['latest']) && isset($this->data[$provider]['mods'][$mod_name]) )
			{
				$this->data[$provider]['mods'][$mod_name] = array_merge($this->data[$provider]['mods'][$mod_name], $mod_data);
			...
			foreach ( $this->data as $provider => $data )
			{
				...
				if ( empty($this->data[$provider]['errno']) && !empty($this->data[$provider]['mods']) )
				{
					foreach ( $this->data[$provider]['mods'] as $mod_name => $mod_data )
					{
						...
						$get->assign_switch('author.mod.dev', !empty($mod_data['dev']));
...
$template->assign_vars(array(
	...
	'L_DEV_VERSION' => $lang['version_dev_info'],
