...
// here we added
//	, forum_as_category
//	, " .  intval($HTTP_POST_VARS['forum_as_category']) . "
//-- modify
			// Begin Simple Subforums MOD
...
//-- mod : forumtitle as weblink -----------------------------------------------
// here we added
//	, title_is_link, weblink, forum_link_icon, forum_link_target
//	, " . intval($HTTP_POST_VARS['forum_is_link']) . ", '" . str_replace("\'", "''", $HTTP_POST_VARS['forum_weblink']) . "', '" . str_replace("\'", "''", $HTTP_POST_VARS['forum_link_icon']) . "', " . intval($HTTP_POST_VARS['forum_link_target']) . "
//-- mod : quick post es -------------------------------------------------------
// here we added
//	, forum_qpes
//	, " . intval($HTTP_POST_VARS['forum_qpes']) . "
//-- modify
			$sql = "INSERT INTO " . FORUMS_TABLE . " (forum_id, forum_name, title_is_link, weblink, forum_link_icon, forum_link_target, forum_as_category, cat_id, forum_parent, forum_desc, forum_order, forum_status, forum_qpes, forum_icon, prune_enable" . $field_sql . ")
				VALUES ('" . $next_id . "', '" . str_replace("\'", "''", $HTTP_POST_VARS['forumname']) . "', " . intval($HTTP_POST_VARS['forum_is_link']) . ", '" . str_replace("\'", "''", $HTTP_POST_VARS['forum_weblink']) . "', '" . str_replace("\'", "''", $HTTP_POST_VARS['forum_link_icon']) . "', " . intval($HTTP_POST_VARS['forum_link_target']) . ", " .  intval($HTTP_POST_VARS['forum_as_category']) . ", " . $new_cat . ', ' . $new_parent . ", '" . str_replace("\'", "''", $HTTP_POST_VARS['forumdesc']) . "', $next_order, " . intval($HTTP_POST_VARS['forumstatus']) . ", " . intval($HTTP_POST_VARS['forum_qpes']) . ", '" . str_replace("\'", "''", $HTTP_POST_VARS['forumicon']) . "', " . intval($HTTP_POST_VARS['prune_enable']) . $value_sql . ")";
//-- fin mod : quick post es ---------------------------------------------------
//-- fin mod : forumtitle as weblink -------------------------------------------
//-- fin mod : forum as category -----------------------------------------------
			if( !$result = $db->sql_query($sql) )
...
//-- mod : forum as category ---------------------------------------------------
// here we added
//	, forum_as_category = " . intval($HTTP_POST_VARS['forum_as_category']) . "
//-- modify
//-- mod : quick post es -------------------------------------------------------
// here we added
				...
				for( $k = 0; $k < $total_forums; $k++ )
				{
					...
					if ( $forum_rows[$k]['forum_parent'] == $forum_id )
					{
						$template->assign_block_vars("catrow.forumrow",	array(
							...
							'FORUM_ICON_IMG' => ( $forum_rows[$k]['forum_icon'] ) ? '<img src="' . $phpbb_root_path . $forum_rows[$k]['forum_icon'] . '" alt="'.$forum_data[$k]['forum_name'].'" title="'.$forum_data[$k]['forum_name'].'" border="0" />' : '', // Forum Icon Mod
							'ROW_COLOR' => $row_color,
//-- mod : forumtitle as weblink -----------------------------------------------
//-- delete
/*-MOD
...
MOD-*/
//-- add
							'NUM_TOPICS' => ( $forum_rows[$k]['forum_link_icon'] != '' && $forum_rows[$k]['title_is_link'] == 1 ) ? (( substr($forum_rows[$k]['forum_link_icon'],0,4) == 'http' || substr($forum_rows[$k]['forum_link_icon'],0,4) == 'HTTP' ) ? '<img src="'.$forum_rows[$k]['forum_link_icon'].'" border="0">' : '<img src="./../'.$forum_rows[$k]['forum_link_icon'].'" border="0">') : $forum_rows[$k]['forum_topics'],
							'NUM_POSTS' => ( $forum_rows[$k]['forum_link_icon'] != '' && $forum_rows[$k]['title_is_link'] == 1 ) ? '' : $forum_rows[$k]['forum_posts'],
//-- fin mod : forumtitle as weblink -------------------------------------------
							'STYLE' => ' style="padding-left: 20px;" ' . '<img src="' . $phpbb_root_path . $images['arrow'] . '" border="0" ',
...
MOD-*/
//-- add
							'U_VIEWFORUM' => ( $forum_rows[$k]['title_is_link'] == 1 ) ? $forum_rows[$k]['weblink'] : append_sid($phpbb_root_path."viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id2"),
//-- fin mod : forumtitle as weblink -------------------------------------------
