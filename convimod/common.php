...
// test to see if the user has just selected a new run stats query list preference and if so, update the db 
if (isset($HTTP_POST_VARS['run_stats_query_option']))
{
	$run_stats_query_option = intval($HTTP_POST_VARS['run_stats_query_option']);
	$sql = "UPDATE " . CONFIG_TABLE . " SET config_value = $run_stats_query_option WHERE config_name = 'run_stats_query_option'";
	if (!$db->sql_query($sql))
	{
		message_die(GENERAL_ERROR, 'Error in updating run_stats_query_option preferences', '', __LINE__, __FILE__, $sql);
	}
}
