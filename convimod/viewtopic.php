...
//	f.auth_cloneposts
//-- Modify
$sql = "SELECT t.topic_id, t.topic_title, t.topic_status, t.topic_replies, t.topic_time, t.topic_type, t.topic_vote, t.topic_last_post_id, f.forum_name, f.forum_status, f.forum_id, f.auth_view, f.auth_read, f.auth_post, f.auth_reply, f.auth_edit, f.auth_delete, f.auth_cloneposts, f.auth_sticky, f.auth_announce, f.auth_pollcreate, f.auth_vote, f.auth_attachments" . $count_sql . "
...
{
	$br_validated = request_var('br_validated', TYPE_INT);
	if ( $br_validated )
	{
		$sql = 'UPDATE ' . USERS_TABLE . ' SET user_rules = ' . BR_APPROVED . ' WHERE user_id = ' . (int) $userdata['user_id'];
		if ( !$db->sql_query($sql, BEGIN_TRANSACTION) )
		{
			message_die(GENERAL_ERROR, 'br_validation_mistake', '', __LINE__, __FILE__, $sql);
		}

		$message = $lang['br_validated'] . '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . $get->url('viewtopic', array('t' => $topic_id), true) . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_forum'], '<a href="' . $get->url('viewforum', array(POST_FORUM_URL => $forum_id), true) . '">', '</a>');
		message_die(GENERAL_MESSAGE, $message, 'Information');
	}

	$u_board_rules = $l_board_rules = $i_board_rules = '';
	if ( ($topic_id == $board_config['br_topic_id']) && ($userdata['user_rules'] == BR_NOT_APPROVED) )
	{
		$u_board_rules = $get->url('viewtopic', array(POST_TOPIC_URL => $topic_id, 'br_validated' => BR_APPROVED), true);
		$l_board_rules = $lang['br_validation_question'];
		$i_board_rules = '';
	}
}

if ( !$board_config['br_required'] || ($userdata['user_rules'] == BR_APPROVED) )
{
	$get->assign_switch('hide_br', true);
}
...
MOD-*/
//-- add
$page_title = $forum_topic_data['topic_title'];
//-- fin mod : quick title edition ---------------------------------------------
//-- fin mod : lo-fi -----------------------------------------------------------
...
if ( $is_auth['auth_mod'] )
{
	...
	if ($lofi)
	{
		$topic_mod .= "<a href=\"modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=delete&amp;sid=" . $userdata['session_id'] . '" title="' . $lang['Delete_topic'] . '">' . $lang['Delete_topic'] . '</a>&nbsp;::&nbsp;';
		
		$topic_mod .= "<a href=\"modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=move&amp;sid=" . $userdata['session_id'] . '" title="' . $lang['Move_topic'] . '">' . $lang['Move_topic'] . '</a>&nbsp;<br />';
	
		$topic_mod .= ( $forum_topic_data['topic_status'] == TOPIC_UNLOCKED ) ? "<a href=\"modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=lock&amp;sid=" . $userdata['session_id'] . '" title="' . $lang['Lock_topic'] . '">' . $lang['Lock_topic'] . '</a>&nbsp;::&nbsp;' : "<a href=\"modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=unlock&amp;sid=" . $userdata['session_id'] . '" title="' . $lang['Unlock_topic'] . '">' . $lang['Unlock_topic'] . '</a>&nbsp;::&nbsp;';
	
		$topic_mod .= "<a href=\"modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=split&amp;sid=" . $userdata['session_id'] . '" title="' . $lang['Split_topic'] . '">' . $lang['Split_topic'] . '</a>&nbsp;';

		$topic_mod .= "<a href=\"modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=merge&amp;sid=" . $userdata['session_id'] . '" title="' . $lang['Merge_topic'] . '" >' . $lang['Merge_topic'] . '</a>&nbsp;';
...
//-- mod : approve board rules -------------------------------------------------
//-- add
	'U_BR_VALIDATE' => $u_board_rules,
	'L_BR_VALIDATE' => $l_board_rules,	
	'I_BR_VALIDATE' => $i_board_rules,
//-- fin mod : approve board rules ---------------------------------------------
...
//-- add
			...
			while($voter = $db->sql_fetchrow($result))
			{
				...
				$voters .= '<a href="'.append_sid('userlist.'.$phpEx.'?mode=viewprofile&u='.$voter['user_id']).'" class="forumlink">'.$voter['username'].'</a>';
			}
			if($voters != '')
         		{   //allow only the admin to view it.
...
//-- fin mod : lo-fi -----------------------------------------------------------
	...
	if ( $user_sig != '' )
	{
		$user_sig = '<br /><img src="' . $images['sig_divide'] . '" alt="User Signature" /><br />' . str_replace("\n", "\n<br />\n", $user_sig);
	}
