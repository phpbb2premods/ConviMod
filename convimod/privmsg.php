...
//Begin Lo-Fi Mod
if ($lofi) {
$lang['Post_new_pm'] = $lang['new_pm_lofi'];
}
//End Lo-Fi Mod
...
//-- fin mod : New then Unread PMs after login ---------------------------------------------------
if ( $mode == 'newpm' )
{
...
//-- fin mod : rank color system -----------------------------------------------
	...
	{
		$private_message .= '<br /><br /><img src="'.$images['sig_divide'].'" /><br />' . make_clickable($user_sig);
	}
...
//Begin Lo-Fi Mod
	if ( $board_config['allow_smilies'] && $privmsg['privmsgs_enable_smilies'] && !$lofi )
//End Lo-Fi Mod
	{
...
//-- mod : View PM while replying MOD, By Manipe ------------------------------------------------
//-- add
			if ( $mode == 'reply' )
			{
				$sql = "SELECT privmsgs_track_id
...
//-- fin mod : View PM while replying MOD, By Manipe --------------------------------------------
		}
		else
		{
...
//-- add
	...
	{
   		if ( $attach_config['allow_pm_attach'] )
   		{
...
-MOD */
//-- replace
	if ( $mode == 'edit' || isset($pm_track_id) )
//-- fin mod : Track PMs MOD ------------------------------------------------- 
	{
		$s_hidden_fields .= '<input type="hidden" name="' . POST_POST_URL . '" value="' . $privmsg_id . '" />';
	}
