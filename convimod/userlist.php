...
//-- mod : mini last visit -----------------------------------------------------
//-- add
			'lastlogin' => array('legend' => 'Sort_Visited', 'field' => 'user_lastlogin'),
//-- fin mod : mini last visit -------------------------------------------------
...
//-- mod : mini last visit -----------------------------------------------------
//-- add
				'USER_LASTLOGIN'=> create_date($lang['DATE_FORMAT'], $this->data[$user_id]['user_lastlogin'], $board_config['board_timezone']),
//-- fin mod : mini last visit -------------------------------------------------
...
//-- mod : mini last visit -----------------------------------------------------
//-- add
				'lastlogin' => !empty($this->user_vars[$user_id]['data']['USER_LASTLOGIN']),
//-- fin mod : mini last visit -------------------------------------------------
	...
	function read()
	{
		...
		$sql = 'SELECT ' . $this->sql_data['fields'] . '
			...
			WHERE ' . $this->sql_data['where'] . 'user_id <> ' . ANONYMOUS
				. ( $this->is_admin ? '' : ' AND user_active = 1' ) . $this->sql_data['group'] . $as_sql . '
...
//-- mod : mini last visit -----------------------------------------------------
//-- add
				'lastlogin' => $this->data[$this->userid]['user_lastlogin'],
//-- fin mod : mini last visit -------------------------------------------------
...
{
	$get->assign_switch('user_is_admin', true);
}
...
//-- mod : approve board rules -------------------------------------------------
//-- add
	'L_USER_RULES' => $lang['Rules'],
	'USER_RULES' => ($profiledata['user_rules'] == BR_APPROVED) ? $lang['User_validated_rules'] : $lang['User_didnot_validated_rules'],
//-- fin mod : approve board rules ---------------------------------------------
...
//-- mod : attach mod ----------------------------------------------------------
//-- add
	'L_ATTACHED_FILES' => $lang['Attached_Files'],
//-- fin mod : attach mod ------------------------------------------------------
...
//-- mod : mini last visit -----------------------------------------------------
//-- add
	'L_LASTLOGIN' => $lang['Visited'],
	'U_SORT_LASTLOGIN' => $get->url($requester, array_merge($parms, array('sort' => 'lastlogin', 'order' => ($parms['order'] == 'd' ? '' : 'd'))), true),
//-- fin mod : mini last visit -------------------------------------------------
